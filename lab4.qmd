...
cont_table_salamander <- table(salamander_clean$section, salamander_clean$unittype)

# Step 3: Chi-square test
chisq_result <- chisq.test(cont_table_salamander)
chisq_result
...
ggpubr::ggviolin(salamander_clean, 
                 x = "section", 
                 y = "weight_g", 
                 add = "boxplot", 
                 color = "section", 
...
```
I reject the null hypothesis that the variances are equal. 
...
```{r}
# Filter for Cutthroat trout and remove missing values
trout_clean <- and_vertebrates |> 
  filter(species == "Cutthroat trout") |> 
  drop_na(length_1_mm, weight_g)

# Run the correlation test
cor.test(trout_clean$length_1_mm, trout_clean$weight_g)
```
We can reject the null hypothesis. There is a strong and statistically significant relationship between the snout-to-fork length and body mass of Cutthroat trout. 
...
pie_sites <- pie_crab |> 
  filter(site %in% c("GTM", "DB", "PIE")) |> 
  mutate(site = factor(site, levels = c("GTM", "DB", "PIE")))

# Create the boxplot, ordered by latitude
ggplot(pie_sites, aes(x = site, y = size, fill = site)) +
  geom_boxplot() +
  geom_jitter(size = 1.5, alpha = 0.7, width = 0.2) + 
  scale_x_discrete(labels = c("GTM", "DB", "PIE")) + 
  labs(x = "Site", y = "Carapace Width (mm)", 
...
water_lm <- lm(size ~ water_temp_sd, data = pie_crab)
...
ggplot(pie_crab, aes(x = water_temp_sd, y = size)) +
  ...
  labs(x = "Water Temperature SD (°C)",
       y = "Carapace Width (mm)",
       title = "Effect of Water Temperature Variability on Carapace Width") +
  theme_minimal()
```
...
multi_lm <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)
